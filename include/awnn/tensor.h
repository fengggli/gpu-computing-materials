...
static inline dim_t make_dim_from_arr(int ndims, int all_dims[]) {
  dim_t ret;
...
void tensor_fill_random_uniform(tensor_t t, double low, double high, int seed);
...
static inline void _add(T *to, T *from, int len) {
  int i;
  for (i = 0; i < len; i++) {
    to[i] += from[i];
  }
}
static inline void _sub(T *to, T *from, int len) {
  int i;
  for (i = 0; i < len; i++) {
...
static inline void _mul(T *to, T *from, int len) {
  int i;
  for (i = 0; i < len; i++) {
    to[i] *= from[i];
  }
}
static inline void _div(T *to, T *from, int len) {
  int i;
  for (i = 0; i < len; i++) {
...
// declare  T *pelem and int i before use
