...
status_t convolution_forward_simple(tensor_t const x, tensor_t const w,
  ...
  int const out_shape[] = {w.dim.dims[0],
  ...
  int const out_shape_2[] = {w.dim.dims[0], y.dim.dims[2], y.dim.dims[3],
  ...
  y.dim = tpose.dim;
  int sz = dim_get_capacity(tpose.dim);
  for (int i = 0; i < sz; ++i) {
...
tensor_t im2col(tensor_t const x, tensor_t const w, conv_param_t const params) {
  ...
  int HH = (H + 2 * pad - filter_height) / stride + 1; // total strides needed over rows
  int WW = (W + 2 * pad - filter_width) / stride + 1;  // total strides needed over cols
...
status_t im2col_inner(tensor_t cols, tensor_t x_padded,
  ...
  for (int c = 0; c < C; c++) // for each channel
    for (int yy = 0; yy < HH; yy++) // stride over rows
      for (int xx = 0; xx < WW; xx++) // stride over cols
        for (int ii = 0; ii < filter_height; ii++) // for each row of filter
          for (int jj = 0; jj < filter_width; jj++){ // for each col of filter
            int row = c * filter_width * filter_height + ii * filter_height + jj;
            for (int i = 0; i < N; i++){
              int col = yy * WW * N + xx * N + i;
              int target_idx = row * cols_d_1 + col;

              int src_idx = (i * img_sz) + (c * chan_sz) + (stride * yy + ii) * row_sz + stride * xx + jj;
...
status_t im2col_inner(tensor_t cols, tensor_t x_padded,
  ...
  int new_img_sz = x_padded.dim.dims[0] * x_padded.dim.dims[1] * x_padded.dim.dims[2] * x_padded.dim.dims[3];
  int channel_sz = x_padded.dim.dims[2] * x_padded.dim.dims[3];
  ...
  for (int n = 0; n < N; n++){
    for (int c = 0; c < C; c++){ // for each channel
      for (int j = 0; j < HH; j++) {  // total strides needed over rows
        for (int k = 0; k < WW; k++) {  // total strides needed over cols
          ...
          for (int f_row = 0; f_row < filter_height; ++f_row) {  // for each row of filter (relative row)
            for (int f_col = 0; f_col < filter_width; ++f_col) {  // for each col of filter
              ...
              int nn = iter / (filters_per_image * filter_size);  // nn is the target image
              int cc = (iter / (filters_per_channel * filter_size)) % C;  // cc is the channel of the target filter
//              int jj = (iter / WW) % HH; // jj is the target filter row
//              int kk = (iter % WW);

              // TODO delete these unused elements
              int t_row = iter / filter_size;
              int t_col = iter % filter_size;
              int t_idx = t_row * filter_size + t_col; // t_idx target index
              assert(t_idx == iter);
              ...
              int ff_row = (iter / filter_width) % filter_width;
              assert(ff_row == f_row);
              int ff_col = iter % filter_width;
              assert(ff_col == f_col);
...
status_t convolution_backward_simple(tensor_t dx, tensor_t dw, lcache_t* cache,
  ...
  int num_filters = w.dim.dims[0];
  int w_channels = w.dim.dims[1];
  int filter_height = w.dim.dims[2];
  int filter_width = w.dim.dims[3];
  ...
  int dout_2d_shape[] = { num_filters, dout_T_1230.dim.dims[1] * dout_T_1230.dim.dims[2] * dout_T_1230.dim.dims[3] };
  ...
  int mult_shape[] = { dout_T_1230.dim.dims[0], x_cols_T.dim.dims[1] };
  ...
  int dw_shape[] = { num_filters, w_channels, filter_height, filter_width };
  ...
  int w_shape[] = { num_filters, w_channels * filter_height * filter_width };
  ...
  int dx_cols_shape[] = { w_T.dim.dims[0], dout_T_1230.dim.dims[1] };
...
# performance quite a bit for col2im.
  ...
  return x_padded
 */
tensor_t col2im(tensor_t dx_cols, int N, int C, int H, int W, int field_height, int field_width, int padding, int stride)
{
  int HH = (H + 2 * padding - field_height) / stride + 1;
  int WW = (W + 2 * padding - field_width) / stride + 1;

  int x_padded_shape[] = { N, C, H + 2 * padding, W + 2 * padding };
...
void col2im_inner(tensor_t dx_cols, tensor_t x_padded, int N, int C, int H, int W, int HH, int WW,
                  int field_height, int field_width, int padding, int stride)
{
  int dx_col_d_1 = dx_cols.dim.dims[1];
  int x_p_d_1 = x_padded.dim.dims[1];
  int x_p_d_2 = x_padded.dim.dims[2];
  int x_p_d_3 = x_padded.dim.dims[3];


  for (int c = 0; c < C; ++c) {
    for (int ii = 0; ii < field_height; ++ii) {
      for (int jj = 0; jj < field_width; ++jj) {
        int row = c * field_width * field_height + ii * field_width + jj;
        for (int yy = 0; yy < HH; ++yy) {
          for (int xx = 0; xx < WW; ++xx) {
            for (int i = 0; i < N; ++i) {
              int col = yy * WW * N + xx * N + i;
              int src_idx = row * dx_col_d_1 + col;
              int target_idx =
...
//                  int field_height, int field_width, int padding, int stride)
//{
//  int dx_col_d1  = dx_cols.dim.dims[1];
...
//    for (int c = 0; c < C; ++c) {  // for each channel
//      for (int fi = 0; fi < field_height; ++fi) {
//        for (int fj = 0; fj < field_width; ++fj) {
//          outer_count++;
//          int row = c * field_width * field_height + fi * field_width + fj;
//
//          int ii = iter / (C * field_height * field_width);
//          int cc = (iter / (field_height * field_width)) % C;  // jj is the channel in the image
//          int fii = iter / (field_width) % field_height;
//          int fjj = iter % field_width;
//
//          for (int h = 0; h < HH; ++h) {
//            for (int w = 0; w < WW; ++w) {
//
                  ...
                  int field_height, int field_width, int padding, int stride)
{
  ...
  int iter = 0;
  for (int ia = 0; ia < N; ++ia) { // for each image
    for (int ca = 0; ca < C; ++ca) {  // for each channel
      for (int fia = 0; fia < field_height; ++fia) {
        for (int fja = 0; fja < field_width; ++fja) {
